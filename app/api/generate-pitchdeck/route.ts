...
import { PitchDeck } from '@/components/copilot/PitchDeck';
...
export async function POST(req: NextRequest) {
  ...
  const element = createElement(PitchDeck, {
    content,
  ...
  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${slug}-pitch-deck-${dateSlug}.pdf"`,
    },
  });
}
